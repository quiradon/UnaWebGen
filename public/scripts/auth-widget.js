...
(function(RPCCloseCodes2) {
  RPCCloseCodes2[RPCCloseCodes2["CLOSE_NORMAL"] = 1000] = "CLOSE_NORMAL";
  ...
  RPCCloseCodes2[RPCCloseCodes2["INVALID_CLIENTID"] = 4000] = "INVALID_CLIENTID";
...
(function(RPCErrorCodes2) {
  RPCErrorCodes2[RPCErrorCodes2["INVALID_PAYLOAD"] = 4000] = "INVALID_PAYLOAD";
...
function zodCoerceUnhandledValue(inputObject) {
  return preprocessType((arg) => {
    var _a;
    const [objectKey] = (_a = Object.entries(inputObject).find(([, value]) => value === arg)) !== null && _a !== undefined ? _a : [];
...
function commandFactory(sendCommand, cmd, response, transferTransform = () => {
  return;
}) {
...
function schemaCommandFactory(cmd, transferTransform = () => {
  return;
}) {
...
function compatCommandFactory({ sendCommand, cmd, response, fallbackTransform, transferTransform = () => {
  return;
} }) {
  const payload = ReceiveFramePayload.extend({
    cmd: literalType(cmd),
...
function getRPCServerSource() {
  var _a;
  return [(_a = window.parent.opener) !== null && _a !== undefined ? _a : window.parent, document.referrer ? document.referrer : "*"];
}
...
class DiscordSDK {
  getTransfer(payload) {
    ...
    switch (payload.cmd) {
      ...
      case Commands.UNSUBSCRIBE:
        return;
      default:
        return (_a = payload.transfer) !== null && _a !== undefined ? _a : undefined;
    }
  }
  constructor(clientId, configuration) {
    ...
    this.sendCommand = (payload) => {
      ...
      const nonce = v4();
      (_a = this.source) === null || _a === undefined || _a.postMessage([Opcodes.FRAME, Object.assign(Object.assign({}, payload), { nonce })], this.sourceOrigin, this.getTransfer(payload));
    ...
    this.configuration = configuration !== null && configuration !== undefined ? configuration : getDefaultSdkConfiguration();
    if (typeof window !== "undefined") {
  ...
  close(code, message) {
    ...
    const nonce = v4();
    (_a = this.source) === null || _a === undefined || _a.postMessage([Opcodes.CLOSE, { code, message, nonce }], this.sourceOrigin);
  }
  ...
  handshake() {
    ...
    (_a = this.source) === null || _a === undefined || _a.postMessage([Opcodes.HANDSHAKE, handshakePayload], this.sourceOrigin);
  }
  addOnReadyListener() {
    ...
    if (parsed.cmd === "DISPATCH") {
      ...
      if (parsed.evt === ERROR) {
        if (parsed.nonce != null) {
          (_a = this.pendingCommands.get(parsed.nonce)) === null || _a === undefined || _a.reject(parsed.data);
      ...
      (_b = this.pendingCommands.get(parsed.nonce)) === null || _b === undefined || _b.resolve(parsed);
      this.pendingCommands.delete(parsed.nonce);
    }
  }
  _getSearch() {
...
function requireLodash_transform() {
  ...
  (function(module, exports) {
    ...
    var { Symbol: Symbol2, Uint8Array: Uint8Array2 } = root, getPrototype = overArg(Object.getPrototypeOf, Object), objectCreate = Object.create, propertyIsEnumerable = objectProto.propertyIsEnumerable, splice = arrayProto.splice;
...
// src/scripts/auth-widget.ts
(async () => {
  if (typeof window === "undefined")
    return;
  const win2 = window;
  if (win2.__krakenAuthWidgetInit) {
    return;
  }
  win2.__krakenAuthWidgetInit = true;
  const initWidget = async (root) => {
    ...
    const loginTemplate = document.querySelector(`template[data-auth-template="login"][data-variant="${variant}"]`);
    const userTemplate = document.querySelector(`template[data-auth-template="user"][data-variant="${variant}"]`);
    ...
    const closeDropdown = () => {
      if (!elements.dropdown || !elements.toggleButton)
        return;
    ...
    const handleOutsideClick = (event) => {
      if (!elements.userBlock || !elements.dropdown || elements.dropdown.hidden)
        return;
    ...
    const renderLoginView = () => {
      ...
      elements.loginAnchor = root.querySelector("[data-auth-login]");
    ...
    const bindUserInteractions = () => {
      if (!elements.toggleButton || !elements.logoutButton)
        return;
      ...
      elements.logoutButton.addEventListener("click", async (event) => {
        event.preventDefault();
    ...
    const renderUserView = (session) => {
      ...
      root.appendChild(fragment);
      elements.userBlock = root.querySelector("[data-auth-user]");
      elements.avatarElement = root.querySelector("[data-auth-avatar]");
      elements.premiumBadge = root.querySelector("[data-auth-premium-badge]");
      elements.nameElement = root.querySelector("[data-auth-name]");
      elements.usernameElement = root.querySelector("[data-auth-username]");
      elements.toggleButton = root.querySelector("[data-auth-toggle]");
      elements.dropdown = root.querySelector("[data-auth-dropdown]");
      elements.logoutButton = root.querySelector("[data-auth-logout]");
      elements.loginAnchor = null;
      if (!elements.userBlock || !elements.avatarElement || !elements.nameElement || !elements.usernameElement || !elements.toggleButton || !elements.dropdown || !elements.logoutButton) {
      ...
      if (elements.premiumBadge && session.premium && session.premium.level >= 1 && session.premium.active) {
        const level = Math.min(Math.max(session.premium.level, 1), 4);
